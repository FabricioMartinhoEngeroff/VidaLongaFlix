...
No Grafana, um dashboard é composto de **painéis (panels)**. Cada painel tem:
- Um **tipo de visualização** (Time series, Stat, Gauge, Bar chart)
- Uma **query** (PromQL para métricas, LogQL para logs, TraceQL para traces)
- Um **título** e configurações de eixos/cores

##### Como criar um painel
1. Grafana → **Dashboards** → **New** → **New Dashboard** → **Add visualization**
2. Selecionar o data source (ex: **Grafana Mimir**)
3. Colar a query PromQL no campo **Metrics browser**
4. Escolher o tipo de visualização no painel direito
5. Salvar com **Ctrl+S** ou o botão **Save dashboard**

---

##### Dashboard 1 — Golden Signals (backend)

> Arquivo: `observability/grafana/provisioning/dashboards/golden-signals.json`

| Painel | Tipo | Query PromQL |
|---|---|---|
| **Latência P95** | Time series | `histogram_quantile(0.95, sum by(le) (rate(http_server_requests_milliseconds_bucket{job="vidalongaflix-backend"}[5m])))` |
| **Latência P99** | Time series | `histogram_quantile(0.99, sum by(le) (rate(http_server_requests_milliseconds_bucket{job="vidalongaflix-backend"}[5m])))` |
| **Taxa de requisições/s** | Time series | `sum(rate(http_server_requests_milliseconds_count{job="vidalongaflix-backend"}[5m]))` |
| **Taxa de erros 5xx (%)** | Time series | `sum(rate(http_server_requests_milliseconds_count{job="vidalongaflix-backend",http_response_status_code=~"5.."}[5m])) / sum(rate(http_server_requests_milliseconds_count{job="vidalongaflix-backend"}[5m])) * 100` |
| **Disponibilidade (%)** | Stat | `sum(rate(http_server_requests_milliseconds_count{job="vidalongaflix-backend",http_response_status_code=~"2.."}[5m])) / sum(rate(http_server_requests_milliseconds_count{job="vidalongaflix-backend"}[5m])) * 100` |
| **Throughput total** | Stat | `sum(increase(http_server_requests_milliseconds_count{job="vidalongaflix-backend"}[1h]))` |

**Quando usar cada tipo de visualização:**

| Tipo | Quando usar | Exemplo |
|---|---|---|
| **Time series** | Tendência ao longo do tempo | Latência, taxa de erros |
| **Stat** | Valor atual, uma única métrica | Disponibilidade (99.8%) |
| **Gauge** | Valor atual com faixa de "bom/ruim" | Error Budget (0–100%) |
| **Bar chart** | Comparar valores entre endpoints | Req/s por rota |
| **Table** | Múltiplas métricas lado a lado | Latência por endpoint |

---

##### Dashboard 2 — JVM Backend

> Arquivo: `observability/grafana/provisioning/dashboards/jvm-backend.json`

| Painel | Tipo | Query PromQL |
|---|---|---|
| **Heap usada (MB)** | Time series | `jvm_memory_used_bytes{area="heap",job="vidalongaflix-backend"} / 1024 / 1024` |
| **Heap máxima (MB)** | Time series | `jvm_memory_max_bytes{area="heap",job="vidalongaflix-backend"} / 1024 / 1024` |
| **Threads ativas** | Time series | `jvm_threads_live_threads{job="vidalongaflix-backend"}` |
| **HikariCP — conexões ativas** | Gauge | `hikaricp_connections_active{job="vidalongaflix-backend"}` |
| **HikariCP — conexões pendentes** | Stat | `hikaricp_connections_pending{job="vidalongaflix-backend"}` |
| **GC pause (ms)** | Time series | `rate(jvm_gc_pause_seconds_sum{job="vidalongaflix-backend"}[5m]) * 1000` |
| **CPU uso (%)** | Gauge | `process_cpu_usage{job="vidalongaflix-backend"} * 100` |

**Configuração de threshold no Gauge (Grafana UI):**
```
Field → Thresholds:
  Verde:    0 – 70%
  Amarelo: 70 – 85%
...
##### Dashboard 3 — SLI/SLO Disponibilidade

> Arquivo: `observability/grafana/provisioning/dashboards/sli-disponibilidade.json`

| Painel | Tipo | Query |
|---|---|---|
| **SLO atual (%)** | Gauge (0–100) | `avg_over_time((sum(rate(http_server_requests_milliseconds_count{http_response_status_code=~"2.."}[5m])) / sum(rate(http_server_requests_milliseconds_count[5m])))[30d:5m]) * 100` |
| **Error Budget restante (%)** | Gauge | `(1 - (1 - avg_over_time((...)[30d:5m])) / (1 - 0.995)) * 100` |
| **Disponibilidade por dia** | Bar chart (por dia) | `avg_over_time((...)[1d:5m]) * 100` |
| **Downtime acumulado (min)** | Stat | `(1 - avg_over_time((...)[30d:5m])) * 30 * 24 * 60` |

---

##### Como navegar do alerta ao trace no Grafana (drilldown)

```
1. Alerta dispara  →  notificação com link direto para o painel afetado
        ↓
2. Abrir o painel  →  ver o pico na Time series no momento do alerta
        ↓
3. Clicar no pico  →  Explore → selecionar "Tempo" como data source
        ↓
4. TraceQL automático:
   { .service.name = "vidalongaflix-backend" && duration > 500ms }
        ↓
5. Clicar em um trace  →  ver todos os spans (waterfall view)
        ↓
6. Clicar em um span  →  botão "Logs for this span" abre o Loki
        ↓
7. LogQL automático:
   {service_name="vidalongaflix-backend"} | trace_id = "abc123..."
        ↓
8. Ver o log de erro exato com timestamp correlacionado ao trace
```

> A conexão Tempo → Loki é habilitada em **Configuration → Data Sources → Tempo → Trace to logs** apontando para o data source Loki com a label `trace_id`.

---

#### Passo 8 — Configurar Alertas

Os alertas no Grafana são definidos em `observability/grafana/provisioning/alerting/rules.yaml`.
...
groups:
  - orgId: 1
    name: vidalongaflix-alerts
    folder: VidaLongaFlix
    interval: 1m   # avaliar a cada 1 minuto
    rules:

      # ─── LATÊNCIA P95 ────────────────────────────────────────────────────
      - uid: latencia-p95-warn
        title: "Latência P95 alta (WARN)"
        condition: C
        for: 5m   # manter violado por 5min antes de disparar (evita falsos positivos)
        annotations:
          summary: "P95 acima de 500ms por 5 minutos"
          description: "Endpoint lento. Verifique traces em: http://localhost:3000/explore"
        labels:
          severity: warning
        data:
          - refId: A
            ...
            model:
              expr: >
                histogram_quantile(0.95,
                  sum by(le) (
                    rate(http_server_requests_milliseconds_bucket{job="vidalongaflix-backend"}[5m])
                  )
                )
          - refId: C
      ...
      - uid: latencia-p95-critical
        title: "Latência P95 crítica (CRITICAL)"
        condition: C
        for: 2m
        labels:
          severity: critical
        annotations:
          summary: "P95 acima de 1000ms — impacto real no usuário"
        data:
          - refId: A
            datasourceUid: mimir
            model:
      ...
      # ─── TAXA DE ERROS 5xx ───────────────────────────────────────────────
      - uid: erros-5xx-critical
        title: "Taxa de erros 5xx > 1% (CRITICAL)"
        condition: C
        for: 2m
        labels:
          severity: critical
        annotations:
          ...
          description: "Verifique logs: {service_name='vidalongaflix-backend'} |= 'ERROR'"
        data:
          - refId: A
            datasourceUid: mimir
            model:
              expr: >
                sum(rate(http_server_requests_milliseconds_count{
                  job="vidalongaflix-backend",
                  http_response_status_code=~"5.."}[5m]))
                /
                sum(rate(http_server_requests_milliseconds_count{
                  job="vidalongaflix-backend"}[5m]))
                * 100
          - refId: C
            datasourceUid: "__expr__"
            model:
      ...
      - uid: heap-jvm-warn
        ...
        data:
          - refId: A
            ...
            model:
              expr: >
                jvm_memory_used_bytes{area="heap", job="vidalongaflix-backend"}
                /
                jvm_memory_max_bytes{area="heap", job="vidalongaflix-backend"}
                * 100
          - refId: C
            datasourceUid: "__expr__"
            model:
      ...
      # ─── HEALTH CHECK ────────────────────────────────────────────────────
      - uid: health-check-down
        title: "Health check falhando (CRITICAL)"
        condition: C
        for: 1m
        labels:
          severity: critical
        annotations:
          summary: "App não responde ao health check"
        data:
          - refId: A
            datasourceUid: mimir
            model:
              # health_check retorna 1 (up) ou 0 (down) via Actuator + Micrometer
              expr: >
                up{job="vidalongaflix-backend"} == 0
          - refId: C
            datasourceUid: "__expr__"
            model:
              type: threshold
...
##### Canal de notificação (contact point) — Webhook para Slack

Em `observability/grafana/provisioning/alerting/contact-points.yaml`:

```yaml
apiVersion: 1
contactPoints:
  - orgId: 1
    name: slack-vidalongaflix
    receivers:
      - uid: slack-webhook
        type: slack
        settings:
          url: "${SLACK_WEBHOOK_URL}"   # variável de ambiente
          title: |
...
Em `observability/grafana/provisioning/alerting/policies.yaml`:

```yaml
apiVersion: 1
policies:
  - orgId: 1
    receiver: slack-vidalongaflix
    group_by: [severity]
    group_wait: 30s       # aguardar 30s antes de enviar (agrupa alertas simultâneos)
    group_interval: 5m    # reenviar a cada 5min se ainda ativo
    repeat_interval: 1h   # lembrete a cada 1h se persistir
    routes:
      - receiver: slack-vidalongaflix
        matchers:
...
##### O que você vê no Grafana quando um alerta dispara

```
Alerting → Alert rules → [regra]
  Estado: Normal → Pending (aguardando "for: 5m") → Firing

  Firing mostra:
  ├── Valor atual da métrica (ex: P95 = 847ms)
  ├── Threshold violado (> 500ms)
  ├── Gráfico com a janela de tempo do disparo
  └── Link para Explore com a query pré-preenchida
```
