...
public class CommentControllerTest {
    ...
    void setUp() {
        ...
        commentDTO = new CommentDTO(commentId, "Great video!", null, userId, videoId);
    }

    @Test
    void shouldCreateComment() throws Exception {
        doNothing().when(commentService).create(any(CommentDTO.class));

        mockMvc.perform(post("/comments").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(new CommentDTO(null, "Great video!", null, userId, videoId)))).andExpect(status().isCreated());
    ...
    void shouldReturnConflictWhenCommentExists() throws Exception {
        doThrow(new DuplicateResourceException("Duplicate comment", "Duplicate comment")).when(commentService).create(any(CommentDTO.class));

        mockMvc.perform(post("/comments").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(new CommentDTO(null, "Great video!", null, userId, videoId)))).andExpect(status().isConflict()).andExpect(content().string("Comment: Duplicate comment"));
